    ...
    struct ObjectByExtruder
    {
        ...
        struct Island
        {
            struct Region {
                ...
                void append(std::string type, const ExtrusionEntityCollection* eec, const ExtruderPerCopy* copy_extruders, unsigned int object_copies_num);


                std::vector<ExtruderPerCopy> infills_per_copy_ids;    // indices of infill.entities that are not part of infill wiping (an element for each object copy)
                std::vector<ExtruderPerCopy> perimeters_per_copy_ids; // indices of infill.entities that are not part of infill wiping (an element for each object copy)
            };
            

            std::vector<Region> by_region;                                    // all extrusions for this island, grouped by regions
            const std::vector<Region>& by_region_per_copy(unsigned int copy, bool wiping_entities = false); // returns reference to subvector of by_region
        ...
        private:
            ...
            bool last_wiping_entities = false;              // last state of wiping_entities parameter
        };
